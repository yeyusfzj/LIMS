...
async function createSample() {
  try {
    ...
    log(`✓ 样品创建成功`, 'green');
    log(`  样品 ID: ${sample.id}`, 'yellow');
    log(`  样品编号: ${sample.sampleNumber}`, 'yellow');
    log(`  当前状态: ${sample.status}`, 'yellow');
...
async function updateSampleStatus(sampleId) {
  try {
    ...
    const sample = response.data.data || response.data;
    log(`✓ 样品状态更新成功`, 'green');
    log(`  新状态: ${sample.status}`, 'yellow');
    
    return true;
  } catch (error) {
    log(`✗ 更新样品状态失败: ${error.message}`, 'red');
    if (error.response) {
      log(`  状态码: ${error.response.status}`, 'red');
      log(`  响应: ${JSON.stringify(error.response.data)}`, 'red');
    }
...
async function getAuditTaskDetail(taskId) {
  try {
    ...
    log(`  当前审核级别: ${task.currentLevel}`, 'yellow');
...
async function performAudit(taskId) {
  try {
    ...
    log('✓ 审核操作成功', 'green');
    log(`  审核结果: ${result.decision || result.result}`, 'yellow');
    log(`  审核意见: ${result.comments || result.comment}`, 'yellow');
    log(`  是否完成: ${result.isComplete ? '是' : '否'}`, 'yellow');
    if (result.nextLevel) {
      log(`  下一审核级别: ${result.nextLevel}`, 'yellow');
    }
...
async function runFullTest() {
  ...
  // 3. 更新样品状态为检测完成
  const updateSuccess = await updateSampleStatus(sample.id);
  if (!updateSuccess) {
    log('\n测试终止：更新样品状态失败', 'red');
    return;
  }
  ...
  // 4. 提交审核
  const taskId = await submitAudit(sample.id);
  if (!taskId) {
    log('\n测试终止：提交审核失败', 'red');
    return;
  }
