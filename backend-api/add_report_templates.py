  ...
  <p>{{conclusion}}</p>
        ...
        for template_data in templates:
            ...
            result = await db.execute(
                select(ReportTemplate).where(ReportTemplate.name == template_data['name'])
            )
            ...
            if not existing:
                ...
                print(f'- 模板已存在: {template_data["name"]}')
        
        print(f'\n报告模板数据添加完成!')
        print(f'已创建 {created_count} 个新模板，共 {len(templates)} 个模板')
